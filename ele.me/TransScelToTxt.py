...
if __name__ == '__main__':
    ...
    dict_dir_path = '/Users/hideto/Downloads/'

    for f in o:
        try:
            deal(dict_dir_path+f)
            # 保存结果
            file_out = open('{0}{1}.txt'.format(dict_dir_path,f), 'w')
            for count, py, word in GTable:
                # GTable保存着结果，是一个列表，每个元素是一个元组(词频,拼音,中文词组)，有需要的话可以保存成自己需要个格式
                # 我没排序，所以结果是按照上面输入文件的顺序
                file_out.write(unicode(word).encode('UTF-8'))  # 最终保存文件的编码，可以自给改
                file_out.write('\n')
            file_out.close()
            GTable = []
        except IOError:
